...
// Mirrors PracticePaperSuccessPage.jsx exactly, swapped to the eBooks
// endpoint/copy — same pattern: no webhook, the session_id Stripe appends
// to successUrl is verified server-side by GET /api/ebook-download before
// any download link is shown.
export default function EbookSuccessPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/ebook-download?session_id=${encodeURIComponent(sessionId)}`);
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #EEF2FF 0%, #F5F7FF 50%, #FFF7ED 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, fontFamily: "'Inter', sans-serif" }}>
      <Helmet><title>Your eBooks | ScholarPrep</title></Helmet>
      <div style={{ maxWidth: 520, width: '100%', background: '#fff', borderRadius: 24, padding: 40, textAlign: 'center', boxShadow: '0 8px 40px rgba(67,56,202,0.1)' }}>
        ...
        {status === 'ready' && (
          <>
            ...
            <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: 22, fontWeight: 800, color: '#0F172A', marginBottom: 8 }}>Your eBooks are ready!</div>
            ...
            <button onClick={() => navigate('/ebooks')} style={{ padding: '11px 24px', borderRadius: 100, fontSize: 14, fontWeight: 600, background: '#F1F5F9', color: '#64748B', border: 'none', cursor: 'pointer' }}>
              Back to eBooks
            </button>
          </>
        )}
