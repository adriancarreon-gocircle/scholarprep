  ...
  return (
    <div style={{ maxWidth: 600, margin: '0 auto', padding: 32 }}>
      <div style={{ marginBottom: 32 }}>
        ...
        <div style={{ fontSize: 15, color: '#64748B', fontFamily: 'Inter, DM Sans, sans-serif' }}>AI-generated questions · Year {yearLevel} level</div>
      </div>
...
function LoadingScreen({ subject }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 400, gap: 20, padding: 32 }}>
      ...
      <div style={{ fontSize: 15, color: '#64748B', maxWidth: 320, textAlign: 'center', lineHeight: 1.7, fontFamily: 'Inter, sans-serif' }}>
            ...
            return (
              <button key={letter} onClick={() => handleSelect(letter)} style={{
                ...
                <div style={{
                  ...
                  background: isRevealed && isCorrect ? '#059669' : isRevealed && isSelected ? '#BE123C' : isSelected ? cfg.color : 'rgba(67,56,202,0.08)',
                  color: (isRevealed && isCorrect) || (isRevealed && isSelected) || isSelected ? '#fff' : '#64748B',
                }}>{letter}</div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: 5 }}>
          {questions.map((_, i) => (
            <div key={i} onClick={() => setCurrent(i)} style={{
              ...
              background: i === current ? '#4338CA' : selected[i] ? (selected[i] === questions[i].correct ? '#059669' : '#F43F5E') : '#E2E8F0',
        ...
        {current < questions.length - 1 ? (
          ...
          <button onClick={handleFinish} style={{
            ...
            fontFamily: 'Inter, sans-serif', boxShadow: '0 4px 12px rgba(249,115,22,0.3)',
          }}>Finish test ✓</button>
        )}
      </div>
    </div>
        ...
        return (
          <div key={i} style={{ background: '#fff', borderRadius: 16, padding: '16px 20px', marginBottom: 10, border: '1px solid rgba(67,56,202,0.06)', display: 'flex', gap: 14, boxShadow: '0 1px 4px rgba(67,56,202,0.04)' }}>
            <div style={{
              ...
              background: correct ? '#ECFDF5' : '#FFF1F2',
              color: correct ? '#059669' : '#BE123C',
              display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 700
            }}>{correct ? '✓' : '✗'}</div>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 14, fontWeight: 500, color: '#0F172A', marginBottom: 6, lineHeight: 1.6, fontFamily: 'Inter, sans-serif' }}>
                <strong>Q{i + 1}.</strong> {q.question}
              </div>
              {!correct && (
                <div style={{ fontSize: 13, color: '#BE123C', marginBottom: 4, fontFamily: 'Inter, sans-serif' }}>
                  You answered: <strong>{userAnswer ? `${userAnswer}. ${q.options[userAnswer]}` : 'Not answered'}</strong>
                </div>
              )}
              <div style={{ fontSize: 13, color: '#059669', marginBottom: 6, fontFamily: 'Inter, sans-serif' }}>
                Correct: <strong>{q.correct}. {q.options[q.correct]}</strong>
              </div>
              {!correct && (
                <div style={{ fontSize: 13, color: '#64748B', background: '#EEF2FF', padding: '10px 14px', borderRadius: 10, lineHeight: 1.7, fontFamily: 'Inter, sans-serif' }}>
                  💡 {q.explanation}
                </div>
              )}
            </div>
    ...
    <div style={{ minHeight: '100vh', background: '#F5F7FF' }}>
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid rgba(67,56,202,0.08)', padding: '20px 32px', display: 'flex', alignItems: 'center', gap: 14 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 13, color: '#94A3B8', marginTop: 2, fontFamily: 'Inter, sans-serif' }}>AI-generated · Year {yearLevel} · Fresh questions every session</div>
        </div>
      </div>
