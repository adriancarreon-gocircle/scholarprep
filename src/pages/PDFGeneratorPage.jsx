  ...
  const handleDownloadPDF = () => {
    ...
    const questionsHtml = subject === 'reading' && passage
      ? `<div class="passage"><h2>${passage.title}</h2><p>${passage.text.replace(/\n\n/g, '</p><p>')}</p></div>
         ${questions.map((q, i) => `<div class="question"><p class="q-text">${i + 1}. ${q.question}</p><div class="options">${Object.entries(q.options).sort((a, b) => a[0].localeCompare(b[0])).map(([l, t]) => `<p>   ${l}. ${t}</p>`).join('')}</div></div>`).join('')}`
      : questions.map((q, i) => `<div class="question"><p class="q-text">${i + 1}. ${q.question}</p><div class="options">${Object.entries(q.options).sort((a, b) => a[0].localeCompare(b[0])).map(([l, t]) => `<p>   ${l}. ${t}</p>`).join('')}</div></div>`).join('');
