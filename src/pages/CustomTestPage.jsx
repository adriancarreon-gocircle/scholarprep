                  ...
                  subj.topics.map(topic => {
                    ...
                    return (
                      <div key={topic.key} style={{ borderBottom: '1px solid #F8FAFC' }}>
                        <div onClick={() => setExpandedTopics(p => ({ ...p, [tKey]: !p[tKey] }))} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 20px 10px 32px', cursor: 'pointer' }}>
                          <span style={{ fontSize: 10, color: isTExp ? subj.color : '#94A3B8' }}>{isTExp ? '▼' : '▶'}</span>
                          <span style={{ flex: 1, fontSize: 14, fontWeight: 600, color: '#0F172A', fontFamily: 'Inter, sans-serif' }}>{topic.label}</span>
                          {tTotal > 0 && <span style={{ fontSize: 12, color: subj.color, fontWeight: 700, fontFamily: 'Inter, sans-serif' }}>({tTotal}q)</span>}
                        </div>
        ...
        if (sk === 'general') {
          ...
          for (const [tk, qtSel] of Object.entries(topicSel)) {
            for (const [qtk, count] of Object.entries(qtSel)) {
              ...
              const focusStr = qtObj ? `${tObj.label} - ${qtObj.label}: ${qtObj.examples?.[0] || ''}` : null;
              const genQs = await generateGeneralAbilityQuestions(yearLevel, count);
        ...
        if (sk === 'mathematics') {
          ...
          for (const [tk, qtSel] of Object.entries(topicSel)) {
            for (const [qtk, count] of Object.entries(qtSel)) {
              ...
              const focusStr2 = qtObj2 ? `${tObj2.label} - ${qtObj2.label}. Example: ${qtObj2.examples?.[0] || ''}` : null;
