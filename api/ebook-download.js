...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (session.metadata?.productType !== 'ebook') {
      return res.status(400).json({ error: 'This session is not an eBook purchase.' });
    }

    const skus = (session.metadata.skus || '').split(',').filter(Boolean);
    if (skus.length === 0) {
      return res.status(404).json({ error: 'No eBooks found for this purchase.' });
    }
    ...
    for (const sku of skus) {
      ...
      const { data, error } = await supabaseAdmin.storage
        .from('ebooks')
        .createSignedUrl(ebook.file, 60 * 60 * 24 * 7); // 7-day link
      if (!error && data) {
        downloads.push({ title: ebook.title, url: data.signedUrl });
      }
    }
